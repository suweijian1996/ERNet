...
def fit_one_epoch(encoder,decoder,dirnet,epoch, epoch_size, epoch_size_val, genir, genvis,gen, Epoch, batch_size,cuda):
    ...
    with tqdm(total=epoch_size, desc=f'Epoch {epoch + 1}/{Epoch}', postfix=dict, mininterval=0.3) as pbar:
        for iteration, batch in enumerate(zip(gen,genir,genvis)):
            ...
            for temp in range(1):
                ...
                fake_ir,fake_vis,_ = decoder(real_feature,x1,x2)
            ...
            loss_idt = l1loss(fake_ir,lir) + l1loss(fake_vis,lvis)
            ...
            l_feature, x1, x2 = encoder(torch.cat((l, l), 1))
            _, _, output = decoder(l_feature.detach(), x1, x2)
            loss_base = 5*l1loss(gradient(output), gradient(l)) + mseloss(output, l)
            loss_base.backward()
            de_optimizer.step()
...
if __name__ == "__main__":
    ...
    if True:
        ...
        annotation_path = '../train_LLVIPIR.txt'
        with open(annotation_path) as f:
            lines = f.readlines()
        # np.random.seed(10101)
        # np.random.shuffle(lines)
        ...
        annotation_path = '../train_LLVIPVIS.txt'
        with open(annotation_path) as f:
            lines = f.readlines()
        # np.random.seed(10101)
        # np.random.shuffle(lines)
